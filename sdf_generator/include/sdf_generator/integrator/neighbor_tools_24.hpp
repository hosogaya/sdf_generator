...
public:
    using LongIndexOffsets = Eigen::Matrix<LongIndexElement, 3, 24>;
    using IndexOffsets = Eigen::Matrix<IndexElement, 3, 24>;
    using Distances = Eigen::Matrix<float, 1, 24>;
...
public:
    ...
    using IndexMatrix = Eigen::Matrix<LongIndexElement, 3, 24>;
    ...
    {  // NOLINT
        ...
        if (coc_index(1) <= global_index(1)) 
        {
            ...
            if (coc_index(2) <= global_index(2)) neighbors_idx.push_back(9);
            if (coc_index(2) >= global_index(2)) neighbors_idx.push_back(15);
        }
        // y <= obsty
        if (coc_index(1) >= global_index(1)) 
        {
            ...
            if (coc_index(2) <= global_index(2)) neighbors_idx.push_back(14);
            if (coc_index(2) >= global_index(2)) neighbors_idx.push_back(8);
        }
        // x >= obstx
        if (coc_index(0) <= global_index(0)) {
            neighbors_idx.push_back(1);
            neighbors_idx.push_back(19);
            if (coc_index(2) <= global_index(2)) neighbors_idx.push_back(11);
            if (coc_index(2) >= global_index(2)) neighbors_idx.push_back(16);
            if (coc_index(1) >= global_index(1)) neighbors_idx.push_back(7);
            if (coc_index(1) >= global_index(1)) neighbors_idx.push_back(13);
        }
        // x <= obstx
        if (coc_index(0) >= global_index(0)) {
            neighbors_idx.push_back(0);
            neighbors_idx.push_back(18);
            if (coc_index(2) <= global_index(2)) neighbors_idx.push_back(17);
            if (coc_index(2) >= global_index(2)) neighbors_idx.push_back(10);
            if (coc_index(1) >= global_index(1)) neighbors_idx.push_back(12);
            if (coc_index(1) >= global_index(1)) neighbors_idx.push_back(6);
        }
    }
